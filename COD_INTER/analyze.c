            ...
            switch (t->tipo.stmt){ 
                case S_Chamada:
                    if(strcmp(t->atrib.nome,"input")==0){
                        ...
                        //atualiza linhas
                        t->tipo_c = *tipo_c;
                        insere_tab_sim(t->atrib.nome,t->numlinha,0,"global",Void,1);
                    }
                    break;
                default:
                     break;
            ...
            switch (t->tipo.exp){
                case E_Id: // verificar escopo
                    if(strcmp(t->atrib.nome,"void")!=0){
                        if (consulta_tab_sim_uso_var(t->atrib.nome, escopo) == -1){
                            Erro = 1;
                            printf("Erro Semântico  na linha %d\n\tVariável %s não declarada\n",t->numlinha,t->atrib.nome);
                        }else if (consulta_tab_sim_uso_var(t->atrib.nome, escopo) == -2){
                            //mesmo escopo
