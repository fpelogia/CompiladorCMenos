...
int consulta_tab_sim_uso_var ( char * nome , char * escopo)
{ int h = hash(nome);
  ListaDeBlocos l =  Tabela_hash[h];
  while ((l != NULL) && (strcmp(nome,l->nome) != 0)) //percorre a lista até ela ser vazia ou achar a var
    l = l->prox;
  if ((l == NULL)) return -1; //nao encontrado, primeira vez declarada
    ...
    if ((strcmp(escopo, l->escopo) != 0) && (strcmp(l->escopo, "global")!=0) ){ //variavel decalarada num escopo diferente
      return -1;
    }else{
...
{ int i;
  fprintf(listing," | Nome da Variavel:  Tipo    Escopo:   Referenciado nas Linhas:\n");
  fprintf(listing," | ----------------  ------  ---------  ------------------------\n");
  for (i=0;i<SIZE;++i)
